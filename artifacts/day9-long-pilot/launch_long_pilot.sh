...
RELEASE_DECISION="$(
  .venv/bin/python -c \
    'import json, sys; from pathlib import Path; print(json.loads(Path(sys.argv[1]).read_text())["decision"])' \
    "${RELEASE_GATE_PATH}"
)"
...
if [[ "${RELEASE_DECISION}" != "GO" ]]; then
  printf 'Refusing start: Sprint-9 release gate is %s\n' "${RELEASE_DECISION}" >&2
  exit 3
fi
if "${DRY_RUN}"; then
  printf 'dry_run=pass\n'
...
mkdir -p -- "${RUN_ROOT}"
{
  printf 'schema_version=crazyflow.sprint9_launch_manifest.v1\n'
